...
void open_loop_nbest_pitch(float *sw, int start, int end, int len, int *pitch, float *gain, int N, char *stack)
{
   ...
   for (i = start; i <= end; i++)
   {
       if (score[i - start] > best_score[N - 1])
       {
           ...
           g = (float)sqrt(g1 * corr[i - start] / (e0 + 10));  // Explicitly cast to float
           if (g > g1)
               g = g1;
           if (g < 0)
               g = 0;
...
)
{
   ...
   
   {
      ...
      for (i = 0; i < gain_cdbk_size; i++)
      {
          ...
          float g0, g1, g2;
          ptr = gain_cdbk + 3 * i;
          g0 = (float)(0.015625 * ptr[0] + 0.5);  // Explicitly cast to float
          g1 = (float)(0.015625 * ptr[1] + 0.5);  // Explicitly cast to float
          g2 = (float)(0.015625 * ptr[2] + 0.5);  // Explicitly cast to float
          ...
          sum -= C[4] * g2 * g1;
          sum -= C[5] * g2 * g0;
          sum -= 0.5f * C[6] * g0 * g0;  // Explicitly cast constants to float
          sum -= 0.5f * C[7] * g1 * g1;  // Explicitly cast constants to float
          sum -= 0.5f * C[8] * g2 * g2;  // Explicitly cast constants to float

          /* If 1, force "safe" pitch values to handle packet loss better */
          if (0) {
              float tot = (float)fabs(ptr[1]);  // Explicitly cast to float
              if (ptr[0] > 0)
                  tot += (float)ptr[0];  // Explicitly cast to float
              if (ptr[2] > 0)
                  tot += (float)ptr[2];  // Explicitly cast to float
              if (tot > 1)
                  continue;
          }
      ...
      gain[0] = (float)(0.015625 * gain_cdbk[best_cdbk * 3] + 0.5);  // Explicitly cast to float
      gain[1] = (float)(0.015625 * gain_cdbk[best_cdbk * 3 + 1] + 0.5);  // Explicitly cast to float
      gain[2] = (float)(0.015625 * gain_cdbk[best_cdbk * 3 + 2] + 0.5);  // Explicitly cast to float
...
float last_pitch_gain)
{
   ...
   gain[0] = (float)(0.015625 * gain_cdbk[gain_index * 3] + 0.5);  // Explicitly cast to float
   gain[1] = (float)(0.015625 * gain_cdbk[gain_index * 3 + 1] + 0.5);  // Explicitly cast to float
   gain[2] = (float)(0.015625 * gain_cdbk[gain_index * 3 + 2] + 0.5);  // Explicitly cast to float
   ...
   if (count_lost && pitch > subframe_offset)
   {
       ...
       if (1) {
           float tmp = count_lost < 4 ? last_pitch_gain : (float)(0.4 * last_pitch_gain);  // Cast to float
           if (tmp > .95f)
               tmp = .95f;  // Ensure .95 is treated as a float
           gain_sum = (float)fabs(gain[1]);  // Cast to float
           if (gain[0] > 0)
               gain_sum += gain[0];
           else
               gain_sum -= 0.5f * gain[0];  // Cast 0.5 to float
           if (gain[2] > 0)
               gain_sum += gain[2];
           else
               gain_sum -= 0.5f * gain[2];  // Cast 0.5 to float
           if (gain_sum > tmp) {
       ...
       if (0) {
           gain_sum = (float)(fabs(gain[0]) + fabs(gain[1]) + fabs(gain[2]));  // Cast to float
           if (gain_sum > .95f) {  // Ensure .95 is treated as a float
               float fact = .95f / gain_sum;  // Cast .95 to float
               for (i = 0; i < 3; i++)
                   gain[i] *= fact;
           }
       }

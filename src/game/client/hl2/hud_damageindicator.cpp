...
void CHudDamageIndicator::DrawDamageIndicator(int side)
{
	...
	int y[4] = { static_cast<int>(m_flDmgY), static_cast<int>(m_flDmgY + insetY), static_cast<int>(m_flDmgTall1 - insetY), static_cast<int>(m_flDmgTall1) };
	byte alpha[4] = { 0, 255, 255, 0 };
	...
	if (side == 1)
	{
		...
		// realign x coords
		x1 = static_cast<int>(GetWide() - x1);
		x2 = static_cast<int>(GetWide() - x2);

		meshBuilder.Color4ub(r, g, b, static_cast<byte>(a * alpha[0] / 255));
		meshBuilder.TexCoord2f(0, 0, 0);
		meshBuilder.Position3f(static_cast<float>(x1), static_cast<float>(y[0]), 0);
		meshBuilder.AdvanceVertex();

		meshBuilder.Color4ub(r, g, b, static_cast<byte>(a * alpha[3] / 255));
		meshBuilder.TexCoord2f(0, 0, 1);
		meshBuilder.Position3f(static_cast<float>(x1), static_cast<float>(y[3]), 0);
		meshBuilder.AdvanceVertex();

		meshBuilder.Color4ub(r, g, b, static_cast<byte>(a * alpha[2] / 255));
		meshBuilder.TexCoord2f(0, 1, 1);
		meshBuilder.Position3f(static_cast<float>(x2), static_cast<float>(y[2]), 0);
		meshBuilder.AdvanceVertex();

		meshBuilder.Color4ub(r, g, b, static_cast<byte>(a * alpha[1] / 255));
		meshBuilder.TexCoord2f(0, 1, 0);
		meshBuilder.Position3f(static_cast<float>(x2), static_cast<float>(y[1]), 0);
		meshBuilder.AdvanceVertex();
	}
	else
	{

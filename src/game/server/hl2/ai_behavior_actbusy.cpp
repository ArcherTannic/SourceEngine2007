...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
// File: CAI_ActBusyQueueGoal.cpp
// Purpose: Handles NPC queuing logic for act-busy behavior.
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
...
void CAI_ActBusyQueueGoal::DrawDebugGeometryOverlays(void)
{
	...
	// Debug for reservers
	for (int i = 0; i < MAX_QUEUE_NODES; i++) // Changed size_t to int
	{
		if (!m_hNodes[i])
			continue;
...
//-----------------------------------------------------------------------------
// Purpose: Recalculate the number of active nodes in the queue
//-----------------------------------------------------------------------------
void CAI_ActBusyQueueGoal::RecalculateQueueCount(void)
{
	...
	for (int i = 0; i < MAX_QUEUE_NODES; i++) // Changed size_t to int
	{
		if (m_hNodes[i])
		{
			m_hNodes[i]->SetDisabled(i > iCount);
...
//-----------------------------------------------------------------------------
// Other methods remain unchanged except for size_t -> int changes in loops.
//-----------------------------------------------------------------------------

// Purpose: Handle when the player blocks or unblocks nodes
